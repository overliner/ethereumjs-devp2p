...
//const devp2p = require('../dist/index')
//const { ETH, Peer } = require('../dist/index')
...
const common = new Common.default({ chain: 'mainnet' })
...
rlpx.on('peer:added', (peer) => {
  ...
  eth.on('message', async (code, payload) => {
    if (code in devp2p.ETH.MESSAGE_CODES) {
...
function onNewBlock(block, peer) {
  ...
  console.log(`New block found ${blockNumber}: ${blockHashHex} (from ${getPeerAddr(peer)})`)
  console.log(
    `----------------------------------------------------------------------------------------------------------`
  )
